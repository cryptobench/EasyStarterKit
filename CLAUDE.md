# EasyStarterKit Plugin - Development Context

## Project Overview
A starter kit plugin for Hytale servers that saves an admin-defined inventory preset and gives it to new players when they join.

## Features
- `/easystarterkit set` - Admin command to save current inventory as the starter kit
- Automatically gives saved inventory to new players on join
...
# Find inventory-related classes
jar -tf HytaleServer.jar | grep -i "inventory"
...
javap -classpath HytaleServer.jar com.hypixel.hytale.server.core.event.events.player.PlayerConnectEvent

# With more detail (private members too)
javap -p -classpath HytaleServer.jar com.hypixel.hytale.server.core.entity.entities.Player
```
...
```json
{
    ...
    "Name": "EasyStarterKit",
    "Version": "1.0.0",
    "Main": "com.easystarterkit.EasyStarterKit",
...
- `PlayerConnectEvent` - player joins (USE THIS FOR STARTER KIT!)
...
perm group add Adventure starterkit.admin
...
### EasyStarterKit Permissions
| Permission | Description |
|------------|-------------|
| `starterkit.admin` | Can use /easystarterkit set command |
...
```bash
mvn clean package
# Copy target/EasyStarterKit-1.0.0.jar to Server/mods/
```
